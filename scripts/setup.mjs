...
async function main() {
  ...
  const secrets = {
    ...
    // Guards POST /internal/reset; the daily cron passes it in the x-reset-token
    // header to rebuild the demo database (see src/server/reset.ts).
    RESET_TOKEN: rand(24),
  };
  ...
  // This is a public demo: default to the well-known demo credentials shown on
  // the site and the /admin login screen (keep in sync with src/lib/demo.ts).
  // Override with ADMIN_EMAIL / ADMIN_PASSWORD for a private deployment.
  const adminEmail = process.env.ADMIN_EMAIL || process.env.DEMO_EMAIL || 'demo@example.com';
  const adminPassword = process.env.ADMIN_PASSWORD || process.env.DEMO_PASSWORD || 'monograph-demo';
  const signup = await api(base, '/table/users/auth/sign-up', {
    method: 'POST', token: adminServiceToken,
    body: {
      username: process.env.ADMIN_USERNAME || process.env.DEMO_USERNAME || 'demo',
      email: adminEmail,
      password: adminPassword,
      passwordConfirm: adminPassword,
      name: process.env.ADMIN_NAME || process.env.DEMO_NAME || 'Demo Admin',
    },
  });
